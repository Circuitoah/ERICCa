...
class Eikonal_Model:
    ...
    def __init__(self):
       #max values for our mesh
        ...
        r_roots, r_weights, r_mapped_roots = mesh_creator(xa =ra, xb =rb ,x_numpoints = self.r_numpoints)
        b_roots, b_weights, b_mapped_roots = mesh_creator(xa =ba, xb =bb ,x_numpoints = self.b_numpoints)
        z_roots, z_weights, z_mapped_roots = mesh_creator(xa =za, xb =zb ,x_numpoints = self.z_numpoints)
        s_roots, s_weights, s_mapped_roots = mesh_creator(xa =sa, xb = sb, x_numpoints = self.numpoints)
        s_theta_roots, s_theta_weights, s_theta_mapped_roots = mesh_creator(xa =s_theta_a, xb = s_theta_b, x_numpoints = self.numpoints_theta)
        t_roots, t_weights, t_mapped_roots = mesh_creator(xa =ta, xb =tb, x_numpoints = self.numpoints)
        t_theta_roots, t_theta_weights, t_theta_mapped_roots = mesh_creator(xa =t_theta_a , xb =t_theta_b, x_numpoints = self.numpoints_theta)
...
#_____________________________________________________________________________________________________________________________________
        ...
        def sigma_R_matter( rho_t,  rho_p  = 0, Gamma = lambda b: np.exp(-b), Model = "OLA"):
        ...
        def sigma_R_pn(rho_t_p, rho_t_n, rho_p_p, rho_p_n, Gamma_pp, Gamma_pn, Gamma_nn , Model = "OLA"):
